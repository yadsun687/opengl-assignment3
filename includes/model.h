...
private:
  ...
  void processNode(aiNode *node, const aiScene *scene) {
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++) {
      ...
      std::cout << "[Mesh loaded] " << mesh->mName.C_Str() << "\n";

      // find whole model's bounding box
      aiAABB &aabb = mesh->mAABB;
      modelAABB.min.x = std::min(modelAABB.min.x, aabb.mMin.x);
      modelAABB.min.y = std::min(modelAABB.min.y, aabb.mMin.y);
      modelAABB.min.z = std::min(modelAABB.min.z, aabb.mMin.z);

      modelAABB.max.x = std::max(modelAABB.max.x, aabb.mMax.x);
      modelAABB.max.y = std::max(modelAABB.max.y, aabb.mMax.y);
      modelAABB.max.z = std::max(modelAABB.max.z, aabb.mMax.z);
  ...
  void extractBoneWeightForVertices(std::vector<Vertex> &vertices, aiMesh *mesh,
    ...
    for (int boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex) {
      ...
      if (m_BoneInfoMap.find(boneName) == m_BoneInfoMap.end()) {
        // add newly found bone to boneInfoMap
        BoneInfo newBoneInfo;
        newBoneInfo.id = m_BoneCounter;
        newBoneInfo.offset = Calculation::convertMatrixToGLMFormat(
...
unsigned int TextureFromFile(const char *path, const string &directory,
  ...
  unsigned char *data = SOIL_load_image(filename.c_str(), &width, &height,
